 ...
 * 在一个 CPU 核执行某个任务时，我们可以在任何时刻暂停当前任务的执行，
 * 并保留当前执行进度。在下次继续执行该任务时，会从之前停留的进度开始继续执行。暂停的时间可以不是整数。
 * 现在，系统有两个 CPU 核，即我们可以同时执行两个任务，
 * 但是同一个任务不能同时在两个核上执行。给定这颗任务树，请求出所有任务执行完毕的最小时间。
 */
public class On12_schedule_on_binary_tree {

    public static double minimalExecTime(BinaryTreeNode<Integer> root) {
